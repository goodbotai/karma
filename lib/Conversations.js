...
function Conversations() {
  ...
  function karmaConversation(err, convo, locale, firstName, lastName) {
    ...
    convo.addQuestion(generateQuickReply(i18next.t(`${locale}:withWhomRelationship`, {withWhom: ''}),
                                             relationshipsObject),
                          goto('skip yes'),
    ...
    const feelsAndEmo = ['upset', 'hostile'];

    feelsAndEmo.map(femo =>
                    convo.addQuestion(generateQuickReply(i18next.t(`${locale}:${femo}`), nums),
                                      femo === 'hostile' ? goto('social concern') :
                                      nextConversation,
                                      { key: `${femo}Femo`},
                                      'A4'));
    ...
    feelsAndEmo.map(femo =>
                    convo.addQuestion(generateQuickReply(i18next.t(`${locale}:${femo}`), nums),
                                      femo === 'hostile' ? goto('social concern spoken') :
                                      nextConversation,
                                      { key: `${femo}Femo`},
                                      'B3'));
