...
function Conversations() {
  ...
  function karmaConversation(err, convo, language, firstName, lastName) {
    ...
    // What's the difference between None and Other?
    const relationships = [
    ...
    convo.addQuestion(
      generateYesNoButtonTemplate(i18next.t(`${language}:withSomeone`), 'yes_with_someone', 'no_with_someone'),
                      [{
    ...
    emotions.map(emo =>
                    convo.addQuestion(generateQuickReply(i18next.t(`${language}:${emo}`), nums5),
