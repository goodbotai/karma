...
function Conversations() {
  ...
  function repeatAnyoneObject(conversation) {
    ...
    try {
       withWhom = responses.social_concern_spoke_to.text;
    } catch (TypeError) {
      withWhom = null;
    }
  ...
  function karmaConversation(err, convo, language, firstName, lastName) {
    ...
    convo.addQuestion(generateQuickReply(i18next.t(`${language}:doing`),
                                             activitiesObject),
                          nextConversation,
                          { key: 'doing' });

    convo.addQuestion(generateYesNoButtonTemplate(i18next.t(`${language}:withSomeone`),
                                                  'yes_with_someone',
                                                  'no_with_someone'),
                      [{
    ...
    convo.addQuestion(
          ...
          [{
            ...
            callback: (_, conversation) => {
              conversation.responses.repeat.withWhom.push(repeatAnyoneObject(conversation));
              conversation.gotoThread('with whom');
              conversation.next();
            },
          }, {
    ...
    emotions.map(emo =>
                    convo.addQuestion(generateQuickReply(i18next.t(`${language}:${emo}`), nums),
                                      emo === 'active' ? goto('social concern') :
                                      nextConversation,
                                      { key: `feeling_${emo}`},
                                      'A4'));
    ...
    emotions.map(emo =>
                    convo.addQuestion(generateQuickReply(i18next.t(`${language}:${emo}`), nums),
                                      emo === 'active' ? goto('social concern spoken') :
                                      nextConversation,
                                      { key: `feeling_${emo}`},
                                      'B3'));
    ...
    convo.addQuestion(generateQuickReply(i18next.t(`${language}:socialConcernSpokeTo`),
                                        relationshipsObject) ,
                      goto('social concern confidant'),
                      { key: 'socialConcernSpokeTo' },
                     'who did you talk to');

    convo.addQuestion(i18next.t(`${language}:socialConcernConfidant`),
    ...
    convo.addQuestion(generateQuickReply(i18next.t(`${language}:socialConcernChange`),
                                         nums),
                      goto('B4f'),
