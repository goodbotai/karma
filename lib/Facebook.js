...
if (!process.env.SENTRY_DSN) {
  console.log('Error: Specify SENTRY_DSN in environment');
  process.exit(1);
}

if (!process.env.environment) {
  console.log('Error: Specify environment in environment');
  process.exit(1);
}
  ...
  logger: new winston.Logger({
    transports: [
      ...
      new (winston.transports.File)({
        filename: process.env.karma_debug_file,
        level: 'debug'
      })
    ]
  }),
...
karma.setupWebserver(process.env.PORT, (err, webserver) => {
  ...
  webserver.use(expressWinston.logger({
    transports: [
      new winston.transports.File({
        filename: 'karma_http.log',
        logstash: true,
      })],
  }));

  karma.createWebhookEndpoints(webserver, karmaProcess, () => {
    console.log(`Karma is listening on port ${PORT}`);
  });
