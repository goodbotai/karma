...
const KarmaConversation = require('../Conversations/KarmaConversation.js');
...
karma.on('facebook_postback', (bot, message) => {
  if (message.payload === 'get_started') {
    bot.createConversation(message, KarmaConversation.perpareConversation);
  }
});

karma.on('facebook_postback', (bot, message) => {
  if (message.payload === 'restart') {
    bot.createConversation(message, KarmaConversation.prepareConversation);
  }
});
