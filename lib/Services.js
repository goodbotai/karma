...
// TODO: Stop replicating vars
const sentryDSN = process.env.SENTRY_DSN;
const environment = process.env.environment;

const facebookApiVersion = process.env.Facebook_api_version;
...
function Services(conversation) {
  ...
  const raven = Services.setupSentry(sentryDSN, environment, 'error');
      ...
      .then((response) => {
        if (response.status < 300) {
          ...
          extractJsonFromResponse(response).then((jsonResponse) => {
            ...
            raven.captureMessage(`RapidPro contact update failed\n{Status: ${response.status}}\n{Payload: ${jsonPayload}}\n{Response: ${jsonResponseString}}`);
            winston.log('error', `{Status: ${response.status}} {Payload: ${jsonPayload}} {Response ${jsonResponseString}}`);
          });
        }
      });
  }
    ...
    fetch(onaSubmissionEndpointURL,
      ...
      .then((response) => {
        if (response.status < 300) {
          ...
          extractJsonFromResponse(response).then((jsonResponse) => {
            let jsonResponseString = JSON.stringify(jsonResponse);
            raven.captureMessage(`Ona submission failed\n{Status: ${response.status}}\n{Payload: ${jsonPayload}}\n{Response ${jsonResponseString}}`);
            winston.log('error', `{Status: ${response.status}} {Payload: ${jsonPayload}} {Response ${jsonResponseString}}`);
          });
        }
      });
...
module.exports = {Services, setupSentry};
