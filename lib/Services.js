...
function Services(conversation) {
  ...
  function genOnaSubmission() {
    ...
    withWhom = responses.repeat.withWhom;
    spoken = responses.repeat.spoken;

    const names = Object.keys(responses);

    const subb = names.map((name) => {
      if ((responses[name].spoken != undefined) && (name === 'repeat')) {
      ...
      if ((responses[name].withWhom != undefined) && (name === 'repeat')) {
        const answer = responses[name].withWhom;
        const kv = {};
        kv.withWhom = answer;
        return kv;
      }
