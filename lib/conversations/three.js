...
function botThree(err, convo, {
  ...
  convo.addQuestion(
    ...
    validateAnswer((txt) => flattenedRelationships.includes(txt.split('/')[0]),
      ...
      if (conversation.status === 'interrupted') {
        ...
        if (f==='switch') {
          ...
          services.getUser({urn: `facebook:${conversation.context.user}`})
            ...
            .catch((reason) => http.genericCatchRejectedPromise(`Failed to getUser in botThree: ${reason}`));
        }
      }
    }));

  shareButton(convo, {uuid, name}, lang, t(`${lang}:thankYouLater`));
  conversationEndActions(bot,
                         message,
                         convo,
                         name,
                         lang,
