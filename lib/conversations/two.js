...
function secondSurvey(
  ...
  convo.addQuestion(
    generateQuickReply(t(`${lang}:secondSurvey.socialConcernHowAffected`), nums10),
    validateAnswer(
      (txt) => /^([1-9]|10)$/.test(txt),
      goto('B3 upset'),
      nums10,
  ...
  convo.addQuestion(
    generateQuickReply(t(`${lang}:secondSurvey.socialConcernChangeScale`), nums10),
    validateAnswer(
      (txt) => /^([1-9]|10)$/.test(txt),
      goto('what changed'),
      nums10,
  ...
  shareButton(convo, {uuid, name}, lang, submissionUUID, t(`${lang}:utils.thankYou`));
  conversationEndActions(
    bot,
    message,
    convo,
