...
function botTwo(err, convo, {
    ...
    emotions.map((emo) => {
      const currentThread = emo === 'active' ? 'default' : `A4 ${emo}`;
        ...
        if (conversation.status === 'interrupted') {
          ...
          if (f==='switch') {
            ...
            services.getUser({urn: `facebook:${conversation.context.user}`})
              ...
              .catch((reason) => http.genericCatchRejectedPromise(`Failed to getUser in botTwo: ${reason}`));
          }
        }
      }));

      shareButton(convo, {uuid, name}, lang);
      conversationEndActions(bot, message, convo, config.onaFormIds.two);
}
