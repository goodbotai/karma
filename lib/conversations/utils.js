...
function initConversation(lang, convo) {
  ...
  const flattenedRelationships =
        relationships.map((v) => v.split('/')).reduce((acc, cur) => {
 ...
 */
function shareButton(convo, {uuid, name}, lang, title) {
  convo.addMessage({
      attachment: {
        type: 'template',
        payload: {
 ...
 */
function conversationEndActions(bot,
                                message,
                                convo,
                                name,
                                language,
  ...
  convo.on('end', (conversation) => {
    ...
    default:
      winston.log('error', `Unhandled coversation end. Status ${conversation.status}`);
    }
  });
}
