  ...
  function sendGreeting({urn, contact_name: contactName, contact}, slaveBot) {
    services.getUser({urn: urn})
      .then(({results: [rapidProContact]}) => {
        let lang =
        localeUtils.lookupISO6391(rapidProContact.language) || config.defaultLanguage;
        let facebookId = urn.split(':')[1];
        bot.startConversation({user: facebookId, channel: facebookId}, (err, convo) => {
          convo.addQuestion(
            generateYesNoButtonTemplate(t(`${lang}:dailyGreeting`),
