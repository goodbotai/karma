...
function helpConversation(bot, message, lang) {
  bot.startConversation(message, (err, convo) => {
    convo.addQuestion(
      generateYesNoButtonTemplate(t(`${lang}:utils.helpMessage`), ['yesPayload', 'noPayload'],
        lang), [{
  ...
  services.getFacebookProfile(message.user)
    .then((profile) => {
      ...
      return services.createUser(
        `${firstName} ${lastName}`,
        localeUtils.pickLanguage(profile), [`facebook:${message.user}`], [config.rapidproGroups[region]], {
          profile_pic,
          locale,
          timezone,
          gender,
