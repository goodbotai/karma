...
function initConversation(lang, convo) {
  const relationships = Object.keys(enTranslation.relationships).map(
    (relationship) => t(`${lang}:relationships.${relationship}`)
  );
...
/**
 * @param {integer} user - Facebook Messenger PSID
 * @return {object} - a RapidPro contact object
 */
async function getContact(user) {
  const {body: {results: [rapidProContact]}} = await services.getUser({
    urn: `facebook:${user}`,
  });
...
/**
 * @param {integer} user - Facebook Messenger PSID
 * @param {string} language - An ISO6392 language code value
 * @return {object} - a RapidPro contact object
 */
async function updateContact(user, language) {
  ...
  try {
    ...
    logger.logRejectedPromise('Failed updateContact in utils ' + e);
  }
  return res.body;
}
 ...
 */
function logConversationStart({user}, {name}) {
  logger.log('info', {
  ...
  try {
    ...
    try {
      ...
      logger.logRejectedPromise(
        'Failed to create contact in createContact' + e
      );
      throw Error('Failed to create contact ' + e);
    }
  }
}
 ...
 * @return {string} - UUID of the submission that referred the user to the bot
 */
